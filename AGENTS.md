...
> 本文档供下次启动 Codex 时快速回忆项目结构、约定与坑位。
> 上次更新：2026-05-22
...
```
localhost_8081/                       # 仓库根
├── .Codex/                          # Codex 本地配置（agents / agent-memory / settings*.json）
...
- 提交规范（参考 `.Codex/settings.json` 历史）：
...
> 📌 **下次会话从这里开始**：先 `git status`、`git log --oneline -10`、读这份 AGENTS.md，
...
> 以下内容于 2026-07-12 从 `CLAUDE.md` 读取并追加，用于保留原 Claude Code 项目记忆。
> 本文件前文的 Codex 版说明仍作为当前执行主版本。
