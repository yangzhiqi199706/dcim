...
function DesignerApp() {
    ...
    const getParameterReplacementSelectionIds = () => {
    ...
    const closeParameterReplacementDialog = () => {
        parameterReplacementRequestRef.current.invalidate();
        setParameterReplacementBox(false);
    ...
    const openParameterReplacementDialog = async () => {
        const ids = getParameterReplacementSelectionIds();
        const context = createParameterReplacementContext(imagesRef.current, ids);
        if (!context.valid) {
            message.warning(t(context.reasonKey));
            return;
        }

        const requestId = parameterReplacementRequestRef.current.begin();
        ...
        try {
            ...
            const device = devices.find((item) => item && (
                String(item.id) === context.deviceId || String(item.SlaveID) === context.deviceId
            ));
            const options = createDeviceParameterOptions(device);
            if (options.length === 0) {
                message.warning(t('parameterReplacement.unavailable'));
                return;
            }
    ...
    const applyParameterReplacement = () => {
        ...
        if (!isParameterReplacementSelectionCurrent(parameterReplacementContext, getParameterReplacementSelectionIds())) {
            message.warning(t('parameterReplacement.selectionChanged'));
        ...
        imagesRef.current = result.shapes;
        setImages(result.shapes);
        history.push(JSON.parse(JSON.stringify(result.shapes)));
    ...
    const closeMasterControlSaveDialog = () => {
                ...
                <main className="designerShell" aria-label={t('auto.k0387')}>
                    ...
                    <DesignerRibbonToolbar
                        ...
                        commandHandlers={{
                            ...
                            saveMasterControl: openMasterControlSaveDialog,
                    ...
                    <div className="layui-layer" id="saveMasterControl" style={showMasterControlBox ? { 'display': 'block' } : { 'display': 'none' }}>
                        ...
                        <span className="layui-layer-setwin" onClick={closeMasterControlSaveDialog}>
                            <Close />
                        </span>
                        <div className="layui-layer-btn">
                            <Button type="primary" onClick={saveMasterControl}>{t('common.confirm')}</Button>
                            <Button onClick={closeMasterControlSaveDialog}>{t('textReplace.cancel')}</Button>
                        </div>
                    </div>
                    ...
                    <div className="layui-layer" style={parameterReplacementBox ? { 'display': 'block' } : { 'display': 'none' }}>
                        ...
                        <div className="layui-layer-btn">
                            <Button type="primary" loading={parameterReplacementLoading} onClick={applyParameterReplacement}>{t('auto.k0202')}</Button>
