;!function (win) {
	...
	Xadmin.prototype.init = function() {
		// var tab_list = this.get_data();//关闭记录历史tab
  ...
  Xadmin.prototype.end = function() {
    ...
    if(!index){
      ...
      //设置初始tab选中 修改权限菜单后 这个无效
      // console.log($('.left-nav #nav>li').eq(index).html())
      var firstnav=$('.left-nav #nav>li').eq(index).children().eq(1).find('li').eq(0);
      // console.log(firstnav.html())
      if(firstnav.find('a').attr('onclick')){
        firstnav.find('a').click();
      }else{
        firstnav.addClass('open').find('a').eq(0).addClass('active');
        firstnav.find('.nav_right').eq(0).html('&#xe6a6;');
        firstnav.find('ul').eq(0).show();
        firstnav.find('li').first().find('a').click();
      }
    }
  };
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh) {
        ...
        if($('.x-iframe').eq(i).attr('tab-id')==id){
            ...
            //监听tab链接，判断动环的全屏按钮
            if(url.indexOf('dcim')>0){
              setTimeout(function(){
                var $ifram = $('#'+id).contents().find('body');
                $ifram.append('<div class="dcim-screen" onclick="BigScreen()"><i class="layui-icon">&#xe622;</i></div>');
              },500)
            }
  ...
  Xadmin.prototype.del_tab = function (id) {
	...
	Xadmin.prototype.add_lay_tab = function(title,url,id) {
		element.tabAdd('xbs_tab', {
	       title: title 
	        ,content: '<iframe tab-id="'+id+'" frameborder="0" src="'+url+'" scrolling="yes" class="x-iframe" id="'+id+'" allowfullscreen="true"></iframe>'
	        ,id: id
	    })
	}
	 ...
	 * @return {[type]}        [description]
	 */
	Xadmin.prototype.open = function (title,url,w,h,id,full,callback) {
		if (title == null || title == '') {
	        var title=false;
	    };
	    if (url == null || url == '') {
...
function setTabmodel(){
  ...
  if(type=='more'){
    //tab超出一行，关闭最前面的tab
    var $more=$('.layui-unselect.layui-tab-bar');
    if($more && $more.attr('lay-stope')=='tabmore'){
        $more.hide();
        $('#tab-title li').eq(1).find('.layui-tab-close').click();
    }
  }else{
    $.each($('#tab-title li'),function(n,val){
      if(!$(val).hasClass('layui-this') && !$(val).hasClass('home')){
        $(val).find('.layui-tab-close').click();
      }
    })
  }
}
//进入全屏
function BigScreen(){
  window.top.location.href='donghuan-dcim-swiper.html';
}
// md5-----------------------------------------------------------------------------------
/*
